...
public class RunsController : ControllerBase
{
	...
	public IActionResult GetNextRun()
	{
		...
		if (nextWeek > 8)
		{
			return Ok(new { Message = "All weeks completed" });
		}
	...
	public async Task<IActionResult> SaveRunWithProgress([FromBody] SaveRunWithProgressDto saveDto)
	{
		...
		// Create UserTrainingProgress entity
		var progress = new UserTrainingProgress
		{
			UserId = userId,
	...
	private (DateTime StartOfWeek, DateTime EndOfWeek) GetCurrentWeekRange()
	{
		...
		daysSinceMonday = daysSinceMonday < 0 ? 6 : daysSinceMonday; // Adjust for Sunday (treated as 0)

		var startOfWeek = today.AddDays(-daysSinceMonday); // Calculate Monday
		var endOfWeek = startOfWeek.AddDays(7).AddTicks(-1); // End of Sunday (inclusive)
	...
	public async Task<IActionResult> GetRecentRunsOfFollowedUsers()
	{
		...
		// Get recent runs from followed users
	...
	public async Task<IActionResult> LikeRun(int id)
	{
		...
		if (like != null)
		{
			_context.Likes.Remove(like); // Unlike if already liked
		}
		else
		{
	...
	public async Task<IActionResult> GetRunsByUserId(string userId, [FromQuery] int page = 1, [FromQuery] int limit = 5)
	{
		...
		// Fetch runs with pagination and additional data
